...
public class FileController {
	...
	public String certScholarshipReadForm(Model model, HttpSession session) {
		// uploadAjax.jsp로 포워딩
	...
	public ResponseEntity<String> uploadAjax01(MultipartFile file, HttpSession session) throws Exception {
		...
		// org.springframework.web.multipart.commons.CommonsMultipartFile@712206e3
	...
	public ResponseEntity<byte[]> displayFile01(String fileName, HttpSession session) throws Exception {
		...
		try {
			...
			if (mType != null) {
				...
				// 바이트배열을 스트링으로 : new
				// String(fileName.getBytes("utf-8"),"iso-8859-1") * iso-8859-1
				// 서유럽언어, 큰 따옴표 내부에 " \" 내용 \" "
				// 파일의 한글 깨짐 방지
	...
	// (# Ajax업로드 처리 매핑) in certScholarship
	// 파일의 한글처리 : produces="text/plain;charset=utf-8"
	@ResponseBody // view가 아닌 data리턴
	@RequestMapping(value = "uploadAjax02", method = RequestMethod.POST, produces = "text/plain;charset=utf-8")
	public ResponseEntity<String> uploadAjax02(MultipartFile file, HttpSession session) throws Exception {

		logger.info(" CON | certScholarship fileupload 시작");
		...
		fileData.setFileUserId(fileUserId);
		fileData.setFileDivision(2); // cerGrad의 division은 1
		fileData.setOriginalFileName(file.getOriginalFilename());
		fileData.setSavedFileName(UploadFileUtils.uploadFile(uploadPath, file.getOriginalFilename(), file.getBytes()));
		...
		logger.info(" CON | certScholarship fileupload 종료");
		return new ResponseEntity<String>(
				UploadFileUtils.uploadFile(uploadPath, file.getOriginalFilename(), file.getBytes()), HttpStatus.OK);
		// -> 최종적으로 return (new) uploadedFileName;
	}
	...
	// (# Ajax 이미지 표시 매핑)
	@ResponseBody // view가 아닌 data리턴
	@RequestMapping("displayFile02")
	public ResponseEntity<byte[]> displayFile02(String fileName, HttpSession session) throws Exception {
