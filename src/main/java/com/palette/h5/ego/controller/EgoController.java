...
*/	
	 ...
	 * //swot 기본 글 읽기
	 * 
	 * @RequestMapping(value="swotRead",method=RequestMethod.GET) public String
	 * swotRead(HttpSession session){
	 * 
	 ...
	 * 
	 * 
	 * //스왓분석 DB를 리턴받음 Swot readSWOT = EgoDAO.readswot(id);
	 * 
	 * if (readSWOT == null) { logger.info("SWOT분석을 읽어온 데이터가 없습니다."); } else {
	 * logger.info("SWOT분석을 읽어온 데이터가 있습니다."); }
	 * 
	 * //읽혀온 스왓을 리턴해줌 session.setAttribute("swotlist", readSWOT);
	 * 
	 * // swotRead 페이지에서 글작성버튼 생성여부 판단
	 * 
	 * logger.info("CON | SWOT 글읽기 종료"); return "ego/swot/swotReadForm"; }
	 * 
	 * //swot 글 작성 폼 이동
	 * 
	 * @RequestMapping(value="swotWriteForm", method=RequestMethod.GET) public
	 * String swotWriteForm(){
	 * 
	 ...
	 * // swot 글 작성
	 * 
	 * @RequestMapping(value="swotWrite", method=RequestMethod.POST) public
	 * String swotWrite(HttpSession session, String swotS, String swotW, String
	 * swotO, String swotT){
	 * 
	 ...
	 * //세선에서 아이디를 받아올 변수 String id = (String)session.getAttribute("loginId");
	 * 
	 * //데이터를 DAO에 넘겨줌 int result = EgoDAO.writeswot(id, swotS, swotW, swotO,
	 * swotT);
	 * 
	 * if (result != 0) { logger.info("CON | SWOT 입력 성공"); }else {
	 * logger.info("CON | SWOT 입력 실패"); }
	 * 
	 * logger.info("CON | SWOT 글 작성 종료"); return "redirect:swot/swotRead"; }
	 * 
	 * // swot 글 수정 폼이동
	 * 
	 * @RequestMapping(value="swotUpdateFrom", method=RequestMethod.POST) public
	 * String swotUpdateForm(){
	 * 
	 ...
	 * // swot 글 수정
	 * 
	 * @RequestMapping(value="swotUpdate", method=RequestMethod.POST) public
	 * String swotUpdate(HttpSession session,String swotS, String swotW, String
	 * swotO, String swotT){ logger.info("CON | SWOT 수정 시작");
	 * 
	 * //세선에서 아이디를 받아올 변수 String id = (String)session.getAttribute("loginId");
	 * 
	 ...
	 * logger.info("CON | SWOT 입력 실패"); }
	 * 
	 * logger.info("CON | SWOT 수정 종료"); return "ego/swot/swotRead"; }
	 * 
	 * 
